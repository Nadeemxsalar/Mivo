...
const DiceFace = memo(({ value }: { value: number }) => {
  // Mobile par chhote dots, Laptop par bade. Exact perfectly round!
  ...
  return (
    // Dynamic responsive gaps: gap-0.5 for mobile, gap-1 for tab, gap-2 for laptop
...
// 2. PREMIUM PLAYER BOX (Solid 3D Block Roll)
const PlayerBox = memo(({ color, name }: { color: Color, name: string }) => {
  ...
  }, [isRobotMode, isActive, color, hasRolled, isAnimating, isRolling, isFastMode]);
          ...
          {isActive ? (
            <motion.div key="active-dice" variants={rollVariants} initial="idle" animate={isRolling ? "rolling" : "idle"} className="w-10 h-10 sm:w-12 sm:h-12 lg:w-16 lg:h-16 absolute z-20" style={{ transformStyle: "preserve-3d" }}>
              <button 
                ...
                className={`relative w-full h-full bg-gradient-to-br from-white via-gray-100 to-gray-300 rounded-lg lg:rounded-xl shadow-[-4px_6px_10px_rgba(0,0,0,0.6)] border-b-[5px] lg:border-b-[8px] border-r-[3px] lg:border-r-[5px] border-gray-400 flex items-center justify-center transition-all z-10 ${(!hasRolled && !isAnimating && !(isRobotMode && color !== 'yellow')) ? 'cursor-pointer hover:-translate-y-1 hover:border-b-[6px] lg:hover:border-b-[10px] hover:shadow-[-4px_8px_15px_rgba(0,0,0,0.5)] active:translate-y-2 active:border-b-[2px] active:border-r-[2px] active:shadow-none' : 'cursor-default opacity-95'}`}
              >
...
const SettingsModal = ({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md p-4 transition-all">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="bg-slate-900 border border-slate-700 p-6 rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.8)] w-full max-w-sm flex flex-col gap-6">
        <h2 className="text-2xl font-black text-white tracking-widest text-center border-b border-slate-700/50 pb-4">⚙️ SETTINGS</h2>
        
        {/* Token Animation Toggle (Clean without unnecessary boxes) */}
    ...
    return (
      <main className="fixed inset-0 w-screen h-[100dvh] bg-[#060D1A] flex items-center justify-center select-none overflow-hidden p-4">
        <style>{`
          ...
          .particle { position: absolute; border-radius: 50%; opacity: 0.15; animation: float 6s ease-in-out infinite; filter: blur(8px); }
        `}</style>
        ...
        <button onClick={() => setIsSettingsOpen(true)} className="absolute top-6 right-6 z-50 p-3 bg-slate-800/80 hover:bg-slate-700 border border-white/10 rounded-full shadow-lg transition-transform hover:scale-110 active:scale-95 text-xl">⚙️</button>
        <SettingsModal isOpen={isSettingsOpen} onClose={() => setIsSettingsOpen(false)} />

        <motion.div initial={{ scale: 0.8, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} className="relative z-10 w-full max-w-lg flex flex-col items-center gap-8">
          ...
          {/* ADVANCED MODE SELECTOR (Ugly Background Removed, Clean Floating Buttons) */}
          ...
          {/* ADVANCED PLAYER SELECTOR (Ugly Background Removed, Clean Circles) */}
          <div className="w-full flex flex-col gap-3 mt-2">
            ...
            <div className="flex justify-center gap-4 sm:gap-6">
              {[2, 3, 4].map(num => (
                <button key={num} onClick={() => setSelectedPlayers(num)} className={`w-14 h-14 sm:w-16 sm:h-16 rounded-full flex items-center justify-center text-lg sm:text-xl font-black transition-all duration-300 border-2 ${selectedPlayers === num ? 'bg-gradient-to-tr from-yellow-400 to-amber-600 border-yellow-300 text-slate-900 shadow-[0_0_20px_rgba(251,191,36,0.6)] scale-110' : 'bg-slate-800/40 border-slate-700 text-slate-400 hover:bg-slate-800 hover:text-white hover:border-slate-500'}`}>
                  {num}
                </button>
              ))}
            </div>
          ...
          <button onClick={() => startGame(selectedPlayers, modeTab === 'COMPUTER')} className="w-full mt-6 p-5 sm:p-6 bg-gradient-to-r from-blue-600 via-indigo-600 to-purple-600 hover:from-blue-500 hover:via-indigo-500 hover:to-purple-500 rounded-[2rem] font-black text-white shadow-[0_15px_30px_rgba(79,70,229,0.4)] transition-all active:scale-95 border border-white/20 tracking-[0.2em] text-lg sm:text-xl flex items-center justify-center gap-3 group">
  ...
  return (
    <main className="fixed inset-0 w-screen h-[100dvh] overflow-hidden flex flex-col items-center justify-center bg-[#060D1A] select-none touch-none py-2 px-3 md:p-6">
      ...
      <div className="absolute top-2 right-2 md:top-6 md:right-6 z-50 flex gap-2">
        <button onClick={() => setIsSettingsOpen(true)} className="px-3 py-1.5 md:px-4 md:py-2 bg-slate-800 text-slate-300 border border-slate-600 rounded-full text-sm md:text-lg font-bold hover:bg-slate-700 transition-colors flex items-center justify-center shadow-lg">⚙️</button>
        <button onClick={exitGame} className="px-3 py-1.5 md:px-4 md:py-2 bg-red-500/20 text-red-400 border border-red-500/30 rounded-full text-[10px] md:text-xs font-bold tracking-wider hover:bg-red-500/40 transition-colors shadow-lg">QUIT</button>
      </div>
