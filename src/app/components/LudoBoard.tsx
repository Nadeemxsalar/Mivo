...
const StarIcon = ({ color }: { color: string }) => (
  <svg viewBox="0 0 24 24" fill={color} className="w-[70%] h-[70%] drop-shadow-md z-0">
    <path stroke="#000" strokeWidth="1" strokeLinejoin="round" d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
  </svg>
);

// Absolute Coordinates for the Main 52-Step Ludo Path
const PATH_COORDS = [
  [6,1],[6,2],[6,3],[6,4],[6,5],     
  [5,6],[4,6],[3,6],[2,6],[1,6],     
  [0,6],[0,7],[0,8],                 
...
export default function LudoBoard() {
  ...
  // Initialize visual positions once
  useEffect(() => {
  ...
  // Watch for store changes and animate step-by-step
  useEffect(() => {
    players.forEach((p: Player) => {
      p.tokens.forEach((t: Token) => {
        ...
        if (currentVisPos !== undefined && currentVisPos !== targetPos) {
          if (targetPos === -1 || currentVisPos === -1) {
            // Killed or just spawned (snap instantly)
            visualPosRef.current[t.id] = targetPos;
            setRenderTick(v => v + 1);
          } else if (targetPos > currentVisPos) {
            // Moving forward: Animate Step by Step
            ...
            const hop = () => {
              ...
              setRenderTick(v => v + 1);
              if (step < targetPos) {
                setTimeout(hop, 180); // Speed of each block jump
              }
            };
            setTimeout(hop, 180);
          } else {
  ...
  const allTokens = useMemo(() => {
    ...
    return tokensArr;
  }, [players, renderTick]); // renderTick forces visual update during animation
  ...
  return (
    <div className="relative w-full h-full aspect-square bg-slate-800 overflow-hidden shadow-inner">
      ...
      <div className="absolute inset-0 grid bg-black/80 z-0" style={{ gridTemplateColumns: 'repeat(15, 1fr)', gridTemplateRows: 'repeat(15, 1fr)', gap: '1px' }}>
      ...
      {/* LAYER 2: PREMIUM SOLID BASES (Hides the Grid completely) */}
      <div className="absolute top-0 left-0 w-[40%] h-[40%] bg-[#E53935] z-10 border-r-2 border-b-2 border-black/50 shadow-[inset_0_0_25px_rgba(0,0,0,0.4)]">
        <div className="absolute top-[16.66%] left-[16.66%] w-[66.66%] h-[66.66%] bg-[#f8f9fa] rounded-2xl shadow-[inset_0_5px_15px_rgba(0,0,0,0.3)] border-[3px] border-black/20" />
      </div>
      <div className="absolute top-0 right-0 w-[40%] h-[40%] bg-[#43A047] z-10 border-l-2 border-b-2 border-black/50 shadow-[inset_0_0_25px_rgba(0,0,0,0.4)]">
        <div className="absolute top-[16.66%] left-[16.66%] w-[66.66%] h-[66.66%] bg-[#f8f9fa] rounded-2xl shadow-[inset_0_5px_15px_rgba(0,0,0,0.3)] border-[3px] border-black/20" />
      </div>
      <div className="absolute bottom-0 left-0 w-[40%] h-[40%] bg-[#1E88E5] z-10 border-r-2 border-t-2 border-black/50 shadow-[inset_0_0_25px_rgba(0,0,0,0.4)]">
        <div className="absolute top-[16.66%] left-[16.66%] w-[66.66%] h-[66.66%] bg-[#f8f9fa] rounded-2xl shadow-[inset_0_5px_15px_rgba(0,0,0,0.3)] border-[3px] border-black/20" />
      </div>
      <div className="absolute bottom-0 right-0 w-[40%] h-[40%] bg-[#FDD835] z-10 border-l-2 border-t-2 border-black/50 shadow-[inset_0_0_25px_rgba(0,0,0,0.4)]">
        <div className="absolute top-[16.66%] left-[16.66%] w-[66.66%] h-[66.66%] bg-[#f8f9fa] rounded-2xl shadow-[inset_0_5px_15px_rgba(0,0,0,0.3)] border-[3px] border-black/20" />
      </div>

      {/* LAYER 3: Holes for Base Tokens */}
        ...
        {Object.entries(BASE_COORDS).map(([color, coords]) =>
          coords.map(([r, c], i) => (
            <div
              ...
              style={{ width: `${100/15}%`, height: `${100/15}%`, top: `${r*(100/15)}%`, left: `${c*(100/15)}%` }}
            >
              <div className="w-[70%] h-[70%] rounded-full bg-black/10 shadow-[inset_0_5px_10px_rgba(0,0,0,0.4)] border-[1.5px] border-black/20" />
            </div>
          ))
        )}
      </div>

      {/* LAYER 4: Center Home Triangles */}
      <div className="absolute top-[40%] left-[40%] w-[20%] h-[20%] z-30 pointer-events-none drop-shadow-xl">
        <div className="relative w-full h-full border-[1.5px] border-black/40 bg-black/80">
      ...
      <div className="absolute inset-0 z-40 pointer-events-none">
        {allTokens.map((t) => {
          ...
          // Actual store position dictates logic, visualPos dictates rendering
          ...
          const isClustered = overlappingTokens.length > 1;

          let shiftX = 0, shiftY = 0, scale = 1;
          if (isClustered) {
            scale = overlappingTokens.length > 2 ? 0.55 : 0.75; 
            shiftX = overlapIndex % 2 === 0 ? -12 : 12;
            shiftY = overlapIndex < 2 ? -12 : 12;
          }

          return (
            <div 
              key={t.id}
              // duration-150 ensures a snappy, realistic sliding hop between cells
              className={`absolute pointer-events-auto transition-all duration-150 ease-linear flex items-center justify-center ${canMove ? 'z-50' : 'z-40'}`}
              style={{
                width: `${100 / 15}%`,
                height: `${100 / 15}%`,
                top: `${t.r * (100 / 15)}%`,
            ...
            >
              <button
                ...
                disabled={!canMove}
                className={`relative w-[85%] h-[85%] rounded-full group transition-transform ${canMove ? 'cursor-pointer hover:scale-110' : 'cursor-not-allowed'}`}
                style={{ animation: canMove ? 'bounce 1s infinite' : 'none' }} 
              >
                <div className={`absolute bottom-[-15%] left-[5%] w-[90%] h-[90%] rounded-full ${tokenThemes[t.playerColor as Color].base} shadow-[0_5px_10px_rgba(0,0,0,0.8)]`} />
                <div className="absolute inset-0 rounded-full border-[2.5px] border-white/90 shadow-inner bg-white flex items-center justify-center z-10">
                  <div className={`w-[75%] h-[75%] rounded-full ${tokenThemes[t.playerColor as Color].top} shadow-[inset_0_-3px_5px_rgba(0,0,0,0.5)] flex items-start justify-center pt-[10%]`}>
                    <div className="w-[50%] h-[25%] bg-white/70 rounded-full blur-[0.5px]" />
                  </div>
                </div>
                {canMove && <div className="absolute -inset-3 border-[4px] border-white rounded-full animate-ping opacity-60 pointer-events-none z-0" />}
              </button>
            </div>
          );
        })}
